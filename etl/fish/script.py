...
df = pd.read_excel(os.path.join(dir_path, 'fish.xlsx'))
...
geographic_range = utils.geographic_range(
    ...
    fao_marine_fishing_areas=df['FAO Marine Fishing Areas:'],
    endemic_status=df['Endemic/Not'])
...
utils.validate_print_result(formed_data=formed_data, num_rows=34, path=dir_path)
